    ...
    [_session dataTaskWithRequest:request
                     completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
                         if (!error) {
                             ...
                             self->_sessionId = [array valueForKeyPath:@"results.sessionId"];
                             NSLog(@"result: %@", self->_sessionId);
                         } else {
                             self->_sessionId = nil;
                             NSLog(@"error: %@", error);
                         }
                         block(self->_sessionId);
                     }];
    [task resume];
}
                    ...
                    completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
                        ...
                        if (newId) {
                            ...
                            [self connect:^(BOOL connected) {
                                self->_sessionId = connected ? self->_sessionId : [NSString string];
                            }];
                        }
                    }];
        [task resume];
 ...
 */
- (void)close:(void(^ const)(void))block
{
    if (self->_sessionId) {
        // Stop live view
        [_stream cancel];

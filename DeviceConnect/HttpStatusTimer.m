    ...
    [_session dataTaskWithRequest:_request
               completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
                   if (!error) {
                       ...
                       self->_state = [array valueForKey:@"state"];
                       self->_fileUri = [array valueForKeyPath:@"results.fileUri"];
                       NSLog(@"result: %@", self->_state);
                   } else {
                       self->_state = @"error";
                       NSLog(@"GetStorageInfo: received data is invalid.");
                   }
                   if (![self->_state isEqualToString:@"inProgress"]) {
